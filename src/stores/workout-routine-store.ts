...
interface WorkoutRoutineActions {
  ...
  // Exercise management
  addExercise: (dayId: string, exercise: Omit<Exercise, 'id' | 'order'>) => void;
  updateExercise: (dayId: string, exerciseId: string, update: Partial<Exercise>) => void;
  ...
  setGenerationProgress: (progress: { step: string; description: string; completed: boolean }[]) => void;
...
export const useWorkoutRoutineStore = create<WorkoutRoutineStore>()(
  subscribeWithSelector((set) => ({
    ...
    updateRoutine: (update) => set((state) => ({
    ...
    // Exercise management
    addExercise: (dayId, exerciseData) => set((state) => {
    ...
    updateExercise: (dayId, exerciseId, update) => set((state) => {
      if (!state.routine) return state;
    ...
    reorderExercises: (dayId, exerciseIds) => set((state) => {
      if (!state.routine) return state;
      ...
      return {
        routine: {
          ...
          days: state.routine.days.map(day => {
            if (day.id === dayId) {
              const exerciseMap = new Map(day.exercises.map(ex => [ex.id, ex]));
    ...
    updateDay: (dayId, update) => set((state) => {
      if (!state.routine) return state;
    ...
    updateUserProfile: (update) => set((state) => ({
    ...
    updateGenerationStep: (step, completed) => set((state) => ({
...
export const useRoutine = () => useWorkoutRoutineStore(state => state.routine);
export const useUserProfile = () => useWorkoutRoutineStore(state => state.userProfile);
export const useIsGenerating = () => useWorkoutRoutineStore(state => state.isGenerating);
export const useGenerationProgress = () => useWorkoutRoutineStore(state => state.generationProgress);
