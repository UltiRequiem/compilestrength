...
import { convertToModelMessages, streamText, UIMessage, stepCountIs, createUIMessageStream, createUIMessageStreamResponse } from 'ai';
import { BODYBUILDING_SYSTEM_PROMPT, bodybuildingTools } from '@/agents/bodybuilding-agent';

export const maxDuration = 60; // 60 seconds for workout generation

export async function POST(req: Request) {
  try {
    const { messages, agentType = 'bodybuilding' }: {
      messages: UIMessage[];
      agentType?: string;
    } = await req.json();
