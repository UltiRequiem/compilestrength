...
import { db } from '@/db';
import { workoutPrograms, workoutDays, exercises, programExercises } from '@/db/schema';
  ...
  async (userId: string) => {
    try {
      ...
      const daysWithLimitedExercises = await Promise.all(
        days.slice(0, 6).map(async (day) => { // Limit to first 6 days
...
export default async function DashboardPage() {
	...
	const weekDays = userRoutines.length > 0 ?
		// If user has routines, show the first routine's days
		userRoutines[0].days.slice(0, 7).map((day: any, index: number) => ({
			day: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][index] || `Day ${index + 1}`,
			workout: day.name,
			status: index === 3 ? "today" : index < 3 ? "completed" : "upcoming"
		})) :
	...
	const recentActivity = userRoutines.length > 0 ?
		userRoutines[0].days.slice(0, 2).flatMap((day: any) =>
			day.exercises.slice(0, 3).map((exercise: any) => ({
				exercise: exercise.name,
				sets: exercise.sets,
				reps: parseInt(exercise.reps.toString().split('-')[0]) || 8,
