...
import { BarChart3, Clock, Calendar, Dumbbell } from 'lucide-react';
...
export function RoutineStats({ routine }: RoutineStatsProps) {
  ...
  const totalSets = routine.days.reduce((total, day) =>
    total + day.exercises.reduce((dayTotal, exercise) => dayTotal + exercise.sets, 0), 0
  );

  // Estimate workout duration (rough calculation)
  const estimatedDuration = routine.days.length > 0
    ? Math.round(routine.days.reduce((total, day) => {
  ...
  const muscleGroupVolume = routine.days.reduce((acc, day) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {topMuscleGroups.length > 0 && (
        <div className="space-y-3">
          <div className="text-xs text-green-600 font-mono">Training Volume Analysis:</div>
          ...
          <div className="space-y-1">
            <div className="text-xs text-green-600 font-mono">All Muscle Groups:</div>
