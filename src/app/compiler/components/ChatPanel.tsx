...
import { useWorkoutRoutineActions, WorkoutRoutine, UserProfile } from '@/stores/workout-routine-store';
    ...
    switch (data.toolName) {
      ...
      case 'setGenerationProgress':
        if (data.result && data.result.steps) {
          console.log('📊 Updating generation progress in store:', data.result.steps);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-green-600 font-mono text-sm space-y-2">
            ...
            <p>&gt; Tell me about your experience, goals, and available equipment to get started.</p>
          </div>
        )}
                ...
                if (part.type === 'tool-call') {
                  return (
                    <div key={index} className="text-yellow-500 text-xs mt-2">
                      &gt; Calling tool: {(part as { toolName?: string }).toolName || 'unknown'}
                    </div>
                  );
                }
        ...
        {(status === 'submitted' || status === 'streaming') && (
          <div className="text-green-600 font-mono text-sm flex items-center gap-2">
            ...
            <span>&gt; {status === 'submitted' ? 'Thinking...' : 'Generating...'}</span>
          </div>
        )}
