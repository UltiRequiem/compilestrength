import { tool } from 'ai';
import { z } from 'zod';
import { WorkoutRoutine, WorkoutDay, Exercise } from '@/stores/workout-routine-store';

// Define schemas for tool inputs
const exerciseSchema = z.object({
  name: z.string().describe('The name of the exercise'),
  muscleGroups: z.array(z.string()).describe('Primary and secondary muscle groups targeted'),
  ...
  weight: z.number().optional().describe('Starting weight in pounds (optional)'),
  notes: z.string().optional().describe('Exercise-specific notes or form cues'),
});

const workoutDaySchema = z.object({
  name: z.string().describe('Name of the workout day (e.g., "Push Day", "Pull Day")'),
  exercises: z.array(exerciseSchema).describe('List of exercises for this day'),
});

const userProfileSchema = z.object({
  experience: z.enum(['beginner', 'intermediate', 'advanced']).describe('Training experience level'),
  goals: z.array(z.string()).describe('Training goals (e.g., muscle_gain, strength, fat_loss)'),
  ...
  timeConstraints: z.object({
    ...
    minutesPerSession: z.number().min(30).describe('Minutes available per session'),
  }),
  physicalLimitations: z.array(z.string()).optional().describe('Any injuries or physical limitations'),
...
const routineSchema = z.object({
  ...
  description: z.string().optional().describe('Brief description of the routine'),
  ...
  difficulty: z.enum(['beginner', 'intermediate', 'advanced']).describe('Routine difficulty level'),
...
export const updateUserProfile = tool({
  description: 'Update or set the user\'s profile information including experience, goals, and constraints',
...
export const createWorkoutRoutine = tool({
  description: 'Create a complete workout routine based on user profile and goals',
...
export const addExercise = tool({
  ...
  inputSchema: z.object({
    dayName: z.string().describe('Name of the workout day to add the exercise to'),
...
export const explainChoice = tool({
  description: 'Provide detailed explanation for exercise selection, programming choices, or routine structure',
  inputSchema: z.object({
    topic: z.string().describe('What to explain (exercise choice, rep range, frequency, etc.)'),
    reasoning: z.string().describe('Detailed explanation of the reasoning behind the choice'),
    evidence: z.string().optional().describe('Scientific evidence or principles supporting the choice'),
  }),
